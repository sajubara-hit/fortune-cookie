...
export default function App(): JSX.Element {
  ...
  const handleCookieClick = () => {
    ...
    if (stage >= 3) return; // 이미 열린 상태면 클릭 무시 (다시뽑기로 리셋 가능)

    // 연타 제한(700ms)
    setIsClickLocked(true);
    setTimeout(() => setIsClickLocked(false), 700);

    // 진동 연출: CSS 진동 클래스 재트리거
    setShakeKey(k => k + 1);

    // 모바일 진동 API 호출 (가능한 경우)
    try {
    ...
    // 5% 확률로 황금운세
        ...
        let chosen: string; // 선택된 운세 문구를 담을 변수 선언

        if (golden) {
            // 황금 운세일 경우, GOLDEN_FORTUNES 배열에서 랜덤 선택
            const idx = Math.floor(Math.random() * GOLDEN_FORTUNES.length);
            chosen = GOLDEN_FORTUNES[idx];
        
            // 황금이면 짧은 웅웅음 재생 (WebAudio API)
            playGoldenHum();

        } else {
            // 일반 운세일 경우, NORMAL_FORTUNES 배열에서 랜덤 선택
            const idx = Math.floor(Math.random() * FORTUNES.length);
            chosen = FORTUNES[idx];
        }
    setSelectedFortune(chosen); // 최종 선택된 문구 저장
    }
    ...
    // 만약 완전히 열렸다면(3) -> 효과 프레임 재생 & 결과 표시
    if (nextStage === 3) {
      // 재생 시작 (effectFrameIndex 0..7를 빠르게 순차 재생)
      //startEffectAnimation();
      // 결과 프레임을 아래에 표시 (딜레이 약간 두고)
      ...
      <main className="fortune-main">
        ...
        >
          <div className="fortune-text">
            {isGolden && <div className="golden-badge">🌟 황금 운세!</div>}
            {/* selectedFortune이 빈 문자열이라도 p 태그는 렌더링됩니다. */}
            <p>{selectedFortune}</p> 
          </div>
